...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"<b>مرحباً {user_name} أهلاً بك في بوت المزاد اختر نوع النشر</b>",
        parse_mode="HTML",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if req_type == "gift":
        if not (
            ...
            await update.message.reply_text("<b>رابط الهدية غير صحيح</b>", parse_mode="HTML")
            return
        ...
        await update.message.reply_text("<b>تم إرسال طلبك للمراجعة</b>", parse_mode="HTML")
    ...
    # ---------- USERNAME ----------
        ...
        await context.bot.send_message(
            chat_id=ADMIN_GROUP_ID,
            text=f"<b>طلب نشر معرف من</b> {user.mention_html()}\nالمعرف {text}\nنوع النشر {'NFT' if publish_type=='nft' else 'عادي'}",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not request:
        await query.edit_message_text("<b>هذا الطلب غير موجود</b>", parse_mode="HTML")
        return
    ...
    if data.startswith("approve_"):
        ...
        if request["type"] == "gift":
            ...
            msg = f"""
<b>Upgraded Gift Soom</b> <a href="{gift_link}">(Details)</a>
...
"""
        ...
        await query.edit_message_text("<b>تمت الموافقة على الطلب</b>", parse_mode="HTML")
        del pending_requests[request_id]

    elif data.startswith("reject_"):
        await context.bot.send_message(
            chat_id=user_id,
            text="<b>تم رفض الطلب راجع الشروط https://t.me/ieexa/3577</b>",
            parse_mode="HTML"
        )

        await query.edit_message_text("<b>تم الرفض</b>", parse_mode="HTML")
        del pending_requests[request_id]
